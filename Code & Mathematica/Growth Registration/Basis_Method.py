...
#=========================================================================================================================
# generate a diffeomorphisms between spheres using the fourier modes m and n
#=========================================================================================================================
def sphere_diffeo(am, an, m, n, theta_1, phi_1):
    '''
    am and an are Fourier coefficients used in calculating the change of coordinates
    '''
    x = am[0]*np.cos(m*theta_1) + an[0]*np.cos(n*phi_1)  + np.cos(phi_1)*np.sin(theta_1)
    y =  am[1]*np.cos(m*theta_1) + an[1]*np.cos(n*phi_1) + np.sin(phi_1)*np.sin(theta_1)
    z = am[2]*np.cos(m*theta_1) + an[2]*np.cos(n*phi_1) + np.cos(theta_1)

    
    plt.scatter(x.flatten(), y.flatten())
    plt.show()
    
    theta_2 =  np.arccos(z/np.sqrt(x**2 + y**2 + z**2))
    phi_2 =  np.arctan2(y, x) 
    
    print((phi_1 - phi_2).flatten().max()/np.pi)
...
m = 4
n = 3
theta_grid = np.linspace(0, np.pi, 15)
phi_grid = np.linspace(-np.pi, np.pi, 15)
theta_1s, phi_1s = np.meshgrid(theta_grid, phi_grid)

theta_2s, phi_2s = sphere_diffeo(am, an, m, n, theta_1s, phi_1s)

x = np.cos(phi_2s)*np.sin(theta_2s)
y = np.sin(phi_2s)*np.sin(theta_2s)

#plt.scatter(theta_1s.flatten(), phi_1s.flatten())
#plt.scatter(theta_2s.flatten(), phi_2s.flatten())

plt.scatter(x.flatten(), y.flatten())
plt.show()

np.savetxt(cwd / "theta_1.npy" , theta_1s, delimiter=',')

np.savetxt(cwd / "theta_2.npy" , theta_2s, delimiter=',')

np.savetxt(cwd / "phi_1.npy" , phi_1s, delimiter=',')

np.savetxt(cwd / "phi_2.npy" , phi_2s, delimiter=',')
